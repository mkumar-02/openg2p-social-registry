...
from ..models.auth import RequestAuth, ResponseAuth
...
@social_registry_api_router.post("/auth", response_model=ResponseAuth)
async def auth(data: RequestAuth, env: Annotated[Environment, Depends(odoo_env)]):
    ...
    # TODO: Add validations if above parameters are not present.

    client = (
        env["g2p.dci.api.client.credential"]
        .sudo()
        .search(
...
@social_registry_api_router.post(
    "/g2p-connect", response_model=RegistrySearchHttpResponse
)
async def g2p_connect_registry_search(
    data: RegistrySearchHttpRequest, env: Annotated[Environment, Depends(odoo_env)]
):

    print("------- testing ----")
    print(env.context)
    print(request.httprequest.headers.get("Authorization"))
    print(data)
    return {"signature": "", "header": {}, "message": {}}
